...
몇년만에 바닐라로 코드를 작성하면서 React의 소중함을 알았다.
특히 반복되는 DOM 탐색, 이벤트 바인딩 등에서 혼잡해지는 코드로 정신이 아득해졌다.
...
간만에 바닐라로 코드를 작성하다 보니, 디렉토리 구성과 함수 분리에 시간 투자를 하지 못했습니다. 어떤 방향으로 리팩토링을 진행해야할지 피드백 부탁드립니다.
이벤트 분리와 등록/해제 시점에 대해 피드백 부탁드립니다.
